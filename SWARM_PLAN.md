...
|---------|----------|-------|
| Primary AI | `https://YOUR_AZURE_ENDPOINT.cognitiveservices.azure.com/openai/responses?api-version=2025-04-01-preview` | `gpt-5.2-codex` |
| Mini/test | Same endpoint | Same model (lighter prompts) |
| Embeddings | `https://YOUR_AZURE_ENDPOINT.cognitiveservices.azure.com/openai/deployments/qrg-embedding-experimental/embeddings?api-version=2023-05-15` | `qrg-embedding-experimental` |

**API Key:** `REDACTED_API_KEY`

**cURL example:**
```bash
curl -X POST "https://YOUR_AZURE_ENDPOINT.cognitiveservices.azure.com/openai/responses?api-version=2025-04-01-preview" \
...
| Field | Value |
|-------|-------|
| Discovery | `https://login.microsoftonline.com/REDACTED_OIDC_TENANT_ID/v2.0/.well-known/openid-configuration` |
| Tenant ID | `REDACTED_OIDC_TENANT_ID` |
| Client ID | `REDACTED_OIDC_CLIENT_ID` |
...
8. Test Azure OpenAI connectivity:
   ...
   curl -s -o /dev/null -w "%{http_code}" -X POST \
     "https://YOUR_AZURE_ENDPOINT.cognitiveservices.azure.com/openai/responses?api-version=2025-04-01-preview" \
     -H "Content-Type: application/json" \
     -H "Authorization: Bearer REDACTED_API_KEY" \
     -d '{"messages":[{"role":"user","content":"ping"}],"max_completion_tokens":5,"model":"gpt-5.2-codex"}'
   ```
9. Test OIDC discovery endpoint: `curl -s "https://login.microsoftonline.com/REDACTED_OIDC_TENANT_ID/v2.0/.well-known/openid-configuration" | python3 -m json.tool`
...
- Add config fields to `backend/app/config.py`:
  ...
  OIDC_CLIENT_ID: str = ""      # REDACTED_OIDC_CLIENT_ID
  OIDC_CLIENT_SECRET: str = ""  # Optional for public client
  OIDC_TENANT_ID: str = ""      # REDACTED_OIDC_TENANT_ID
  ```
...
- Add model registry entries for:
  - `gpt-5.2-codex` at `https://YOUR_AZURE_ENDPOINT.cognitiveservices.azure.com/openai/responses?api-version=2025-04-01-preview`
  - `qrg-embedding-experimental` at `https://YOUR_AZURE_ENDPOINT.cognitiveservices.azure.com/openai/deployments/qrg-embedding-experimental/embeddings?api-version=2023-05-15`
- API key: `REDACTED_API_KEY` (store in Vault or env var, never hardcode)
...
- Dedicated test that calls the real Azure OpenAI API:
  ...
  def test_azure_openai_direct():
      resp = httpx.post(
          "https://YOUR_AZURE_ENDPOINT.cognitiveservices.azure.com/openai/responses?api-version=2025-04-01-preview",
          headers={"Authorization": "Bearer REDACTED_API_KEY", "Content-Type": "application/json"},
...
- Also test embeddings:
  ...
  def test_azure_embeddings():
      resp = httpx.post(
          "https://YOUR_AZURE_ENDPOINT.cognitiveservices.azure.com/openai/deployments/qrg-embedding-experimental/embeddings?api-version=2023-05-15",
          headers={"Authorization": "Bearer REDACTED_API_KEY", "Content-Type": "application/json"},
...
- Azure OpenAI API key: REDACTED_API_KEY (use as env var, not hardcoded)
- OIDC discovery: https://login.microsoftonline.com/REDACTED_OIDC_TENANT_ID/v2.0/.well-known/openid-configuration
- OIDC client ID: REDACTED_OIDC_CLIENT_ID
