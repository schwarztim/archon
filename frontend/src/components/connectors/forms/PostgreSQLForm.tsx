  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <div>
        ...
        <Input placeholder="localhost" value={config.host ?? ""} onChange={(e) => set("host", e.target.value)} className="bg-gray-50 dark:bg-[#0f1117]" />
      </div>
      <div>
        <label className="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">Port *</label>
        <Input type="number" placeholder="5432" value={config.port ?? "5432"} onChange={(e) => set("port", e.target.value)} className="bg-gray-50 dark:bg-[#0f1117]" />
      </div>
      <div>
        <label className="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">Database *</label>
        <Input placeholder="mydb" value={config.database ?? ""} onChange={(e) => set("database", e.target.value)} className="bg-gray-50 dark:bg-[#0f1117]" />
      </div>
      <div>
        <label className="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">Username *</label>
        <Input placeholder="postgres" value={config.username ?? ""} onChange={(e) => set("username", e.target.value)} className="bg-gray-50 dark:bg-[#0f1117]" />
      </div>
      <div>
