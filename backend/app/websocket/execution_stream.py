...
Implements the full execution streaming lifecycle with:
- Connection management (accept, register, remove)
- Redis Streams persistence (XADD / XRANGE) for durable event replay
- In-memory event buffer (ring buffer, capped at 200 events) as fast-path
  for replay when Redis is unavailable or the stream has been cleared
- Server-side heartbeat / ping-pong every 30 s
- Tenant isolation for broadcast scoping
- Auto-cleanup of dead WebSocket connections on send failure

Event format
------------
Every event broadcast through this manager has the shape::
...
class ExecutionStreamManager:
    ...
    2. ``broadcast()``— persist to Redis, then fan-out to connected sockets
    ...
    async def broadcast(
        ...
        *,
        ...
        2. Store in Redis stream (XADD) — best-effort.
        3. Store in in-memory ring buffer.
    ...
    # Convenience alias used by existing code that calls send_event()
    async def send_event(
        ...
        *,
        ...
        """Alias for :meth:`broadcast` — keeps API compatible with ConnectionManager."""
