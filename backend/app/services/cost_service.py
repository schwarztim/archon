...
def _can_read_all_costs(user: AuthenticatedUser) -> bool:
    ...
    return bool(set(user.roles) & _COST_READ_ROLES) or check_permission(user, "costs", "read")
...
class CostService:
    ...
    async def set_budget(
        ...
        await AuditLogService.create(
            ...
            details={"name": budget.name, "scope": budget.scope, "limit_usd": budget.limit_amount},
        )

        logger.info(
            "Budget created",
            extra={"tenant_id": tenant_id, "budget_id": str(budget.id), "scope": budget.scope},
        )
    ...
    async def check_budget(
        ...
        for b in budgets:
            ...
            usage_pct = (projected / b.limit_amount * 100) if b.limit_amount > 0 else 0.0
    ...
    async def generate_chargeback_report(
        ...
        await AuditLogService.create(
            ...
            details={"period_since": since.isoformat(), "period_until": until.isoformat(), "total": total},
        )
    ...
    async def forecast_costs(
        ...
        """Simple trend-based cost projection from recent daily spend."""
        now = _utcnow()
    ...
    async def get_optimization_recommendations(
        ...
        expensive_models = {"gpt-4o", "gpt-4-turbo", "claude-3-opus", "claude-3-5-sonnet"}
        cheap_alternatives = {"gpt-4o-mini", "gpt-3.5-turbo", "claude-3-haiku", "gemini-1.5-flash"}
    ...
    async def reconcile_provider_invoice(
        ...
        ledger_total = 0.0
        for e in entries:
            ledger_by_model[e.model_id] = ledger_by_model.get(e.model_id, 0.0) + e.total_cost
        ...
        match_pct = (
            ...
            if invoice_total > 0 else (100.0 if ledger_total == 0 else 0.0)
        )
